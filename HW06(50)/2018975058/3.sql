...
declare
    ...
    not_null_exception exception;
    nothing_parents_key exception;
    ...
    pragma exception_init(not_null_exception, -1400);
    pragma exception_init(nothing_parents_key, -2291);
    pragma exception_init(integrity_constraint_violation, -1);
    ...
    for i in test_n.first..test_n.last loop
        ...
        exception
            when child_record_found then
                dbms_output.put_line(i||' : 자식 레코드 발견 !');
            when not_null_exception then
                dbms_output.put_line(i||' : NOT NULL 타입에 NULL을 넣을 수 없습니다.');
            when nothing_parents_key then
                dbms_output.put_line(i||' : 부모 키가 존재하지 않습니다.');
            when integrity_constraint_violation then
                dbms_output.put_line(i||' : 무결성 제약 조건에 위반되었습니다.');
                when check_constraint_violation then
                dbms_output.put_line(i||' : 체크 제약 조건에 위반되었습니다.');
        end;
    end loop;
